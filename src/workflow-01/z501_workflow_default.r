# Corrida general del Workflow Default
...
DT_incorporar_dataset_competencia2024 <- function()
{
  ...
  param_local$semilla <- NULL  # no uso el random
...
CA_catastrophe_default <- function( pinputexps, metodo )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija
...
FEintra_default <- function( pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija
...
#------------------------------------------------------------------------------
# Data Drifting Defauilt
# deterministico, SIN random

DR_drifting_default <- function( pinputexps, metodo)
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija
...
FEhist_default <- function( pinputexps)
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija
...
#------------------------------------------------------------------------------
#  Agregado de variables de Random Forest
#  atencion, parmetros para generar variables, NO para buen modelo
#  azaroso, utiliza semilla

FErf_ranger_attributes_default <- function(pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija

  param_local$meta$script <- "/src/workflow-01/z532_FE_rfatributes_ranger.r"
...
FErf_attributes_default <- function( pinputexps, ratio, desvio)
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija


  param_local$meta$script <- "/src/workflow-01/z533_FE_rfatributes.r"
...
CN_canaritos_asesinos_default <- function( pinputexps, ratio, desvio)
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija
...
TS_strategy_default_202109_limpio <- function( pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija
...
TS_strategy_default_202109 <- function( pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija
...
# Hyperparamteter Tuning default
...
HT_tuning_default <- function( pinputexps, bypass=FALSE)
{
...
HT_tuning_leve <- function( pinputexps, bypass=FALSE)
{
  if( -1 == (param_local <- exp_init(pbypass=bypass))$resultado ) return( 0 ) # linea fija bypass
...
#------------------------------------------------------------------------------
# proceso ZZ_final  default
#  azaroso, utiliza semilla

ZZ_final_default <- function( pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija
...
#------------------------------------------------------------------------------
# Este es el  Workflow de default
# Que predice 202109
# y ya genera archivos para Kaggle

corrida_default_202109 <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija

  DT_incorporar_dataset_competencia2024()

  CA_catastrophe_default( metodo="MachineLearning")
  FEintra_default()
  DR_drifting_default(metodo="rank_cero_fijo")
  FEhist_default()
  FErf_attributes_default()
  CN_canaritos_asesinos_default(ratio=0.2, desvio=4.0)

  ts1 <- TS_strategy_default_202109()
  ht1 <- HT_tuning_default()

  # El ZZ depente de HT y TS , en ese orden
  ZZ_final_default( c(ht1, ts1) )

  return( exp_wf_end() ) # linea fija
}
#------------------------------------------------------------------------------

corrida_default_202109_limpio <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
  ...
  ts1 <- TS_strategy_default_202109_limpio()
  ht1 <- HT_tuning_default()
...
corrida_default_202109_leve <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
...
corrida_default_202109_bypass <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
  ...
  ts1 <- TS_strategy_default_202109_limpio()
  ht1 <- HT_tuning_leve(bypass=TRUE)
...
corrida_basica <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
...
res1 <- corrida_basica( "basica" )
res2 <- corrida_basica( "basica" )
res3 <- corrida_basica()



exp_lib <- paste0( envg$EXPENV$repo_dir,"/src/lib/z690_exp_lib_02.r")
source( exp_lib )

res7 <- corrida_default_202109()

exp_lib <- paste0( envg$EXPENV$repo_dir,"/src/lib/z690_exp_lib_02.r")
source( exp_lib )

res8 <- corrida_default_202109_limpio()


exp_lib <- paste0( envg$EXPENV$repo_dir,"/src/lib/z690_exp_lib_02.r")
source( exp_lib )

res8 <- corrida_default_202109_leve()
...
res8 <- corrida_default_202109_bypass()
...
corrida_manual_202109 <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
  ...
  DR_drifting_default(metodo="rank_cero_fijo")
  FEhist_default()
  FErf_ranger_attributes_default()
  CN_canaritos_asesinos_default(ratio=0.2, desvio=4.0)
...
quasi_202109 <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija
